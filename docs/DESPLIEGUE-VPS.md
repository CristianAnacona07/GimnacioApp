...
No reutilices contraseñas entre `JWT_SECRET`, `MONGO_PASSWORD` y
...
docker compose -f docker-compose.prod.yml exec mongo \
  ...
  --authenticationDatabase admin --db gymdb --out /respaldos/$(date +%F)
# queda en ./deploy/respaldos/ — bajalo del servidor con scp
```
...
Mongo no está expuesto. Abrí un túnel por SSH y conectá Compass a
`localhost:27017`:

```bash
ssh -L 27017:localhost:27017 root@LA-IP -N
```
